...
<script>
  $(function () {
    // Fungsi untuk memperbarui grafik berdasarkan tahun dan status yang dipilih
    function updateChart(year, status) {
      // Ganti URL di bawah ini sesuai dengan URL untuk mengambil data grafik dari server
      var dataURL = '<?= base_url('grafik/getDataStatus/') ?>' + year + '/' + status;
      ...
      $.ajax({
        ...
        success: function (data) {
            ...
            barChartData.labels = data.statusLabels;
            barChartData.datasets[0].data = data.statusData;
            ...
            statusChart = new Chart(barChartCanvas, {
              type: 'bar',
              data: barChartData,
            ...
            console.log('Grafik diperbarui dengan data tahun: ' + year + ', status: ' + status);
        },
    ...
    $('#submit-btn').on('click', function () {
      ...
      updateChart(selectedYear, selectedStatus);
    });
    ...
    // Inisialisasi grafik
    var statusChart = new Chart(barChartCanvas, {
      type: 'bar',
      data: barChartData,
      options: barChartOptions
    });
